...
>(({ className, ...props }, ref) => (
    <tfoot
        ref={ref}
        className={`border-t bg-zinc-50 font-medium [&>tr]:last:border-b-0 ${className ?? ""}`}
        {...props}
    />
))
...
>(({ className, ...props }, ref) => (
    <th
        ref={ref}
        className={`h-11 px-4 text-left align-middle text-xs font-semibold text-zinc-500 uppercase tracking-wide [&:has([role=checkbox])]:pr-0 ${className ?? ""}`}
        {...props}
    />
))
